	...
	public Agent(String name) throws IOException, TimeoutException{
		...
		con = new DefaultConsumer(rabbitClient.getChannel()){
			...
			public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
				...
				JSONObject json;
				if(Util.bytesToIntent(body).name != null){
					//if the name is not null, pass it to the intent resolver
					json = intentResolver(Util.bytesToIntent(body));
				}else{
					//if the name is null, i.e.: the message is not an intent, pass it to the pending resolver
